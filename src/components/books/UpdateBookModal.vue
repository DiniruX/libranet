  ...
  <div>
    ...
    <div class="px-6 py-4 text-left modal-content">
      ...
      <div class="flex items-center justify-between pb-3">
        <p class="text-2xl font-bold">Update Book</p>
      ...
      <label class="block">
        ...
        <input
          ...
          accept="image/*"
        />
        <img v-if="cover_image && !isImageUpdated" :src="cover_image ? `data:image/png;base64,${cover_image}` : ''" class="mb-6 w-32 h-32 object-cover rounded-md" />
        <img v-if="cover_image && isImageUpdated" :src="cover_image" class="mb-6 w-32 h-32 object-cover rounded-md" />
      </label>
...
const success = ref('')
const error = ref('')
const isImageUpdated = ref(false)
...
async function fetchBook() {
  try {
    ...
    if (book) {
      ...
      library_id.value = book.library_id || ''
    }
  } catch (error) {
...
async function updateBook() {
  ...
  try {
    ...
    await axios.put(`${BASE_URL}/books/${props.id}`, formData, {
      headers: {
    ...
    if (axios.isAxiosError(err)) {
      error.value = err.response?.data?.message || 'An error occurred while updating the book'
    } else {
